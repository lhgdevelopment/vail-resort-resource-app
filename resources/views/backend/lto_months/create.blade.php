...
<div class="col-lg-12"> 
    <div class="card">
      <div class="card-body">
        ...
        <form action="{{ isset($ltoMonth) && isset($ltoMonth->id) ? route('lto_months.update', $ltoMonth->id) : route('lto_months.store') }}" method="POST">
            @csrf
            @if(isset($ltoMonth) && isset($ltoMonth->id))
                @method('PUT')
            @endif
            ...
            <div class="mb-3">
                ...
                <select name="status" id="status" class="form-control" required>
                    <option value="1" {{ (isset($ltoMonth) && $ltoMonth->status == 1) || old('status') == '1' ? 'selected' : '' }}>Active</option>
                    <option value="0" {{ (isset($ltoMonth) && $ltoMonth->status == 0) || old('status') == '0' ? 'selected' : '' }}>Inactive</option>
                </select>
            </div>
